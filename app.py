...
def minhas_tabelas():
    ...
    if table_data:
        ...
        if not visible_cols and 'Objeto' in all_columns_in_data:
            visible_cols = ['Objeto']
    return render_template( 'minha_tabela.html', active_table_name=active_table_name, saved_tables_names=list(session['saved_tables'].keys()), table_data=table_data, table_headers=visible_cols, changes=changes)
...
def download_table():
    active_table_name = session.get('active_table', 'Tabela')
    table_data = session['saved_tables'].get(active_table_name, [])
    if not table_data:
        flash("A tabela está vazia, não há nada para baixar.", "warning")
...
def reorder_rows():
    ...
    if not active_table_name or not new_order:
        return jsonify({'success': False, 'message': 'Dados inválidos.'}), 400
    table_list = session['saved_tables'].get(active_table_name, [])
    row_map = {str(row['Objeto']): row for row in table_list}
    reordered_list = [row_map[obj_name] for obj_name in new_order if obj_name in row_map]
    reordered_set = set(new_order)
    for row in table_list:
        if str(row['Objeto']) not in reordered_set:
            reordered_list.append(row)
    session['saved_tables'][active_table_name] = reordered_list
    session.modified = True
...
def rename_table():
    ...
    if old_name and new_name and new_name not in session['saved_tables']:
        session['saved_tables'][new_name] = session['saved_tables'].pop(old_name, [])
        session.modified = True
        session['active_table'] = new_name
...
def delete_table():
    ...
    if table_to_delete and table_to_delete in session['saved_tables']:
        if len(session['saved_tables']) <= 1:
            ...
            return redirect(url_for('minhas_tabelas'))
        session['saved_tables'].pop(table_to_delete)
        session.modified = True
...
def delete_rows():
    active_table_name = session.get('active_table')
    table_list = session['saved_tables'].get(active_table_name, [])
    rows_to_delete_str = request.form.get('rows_to_delete', '')
    rows_to_delete = {name.strip() for name in rows_to_delete_str.split(',') if name.strip()}
    if rows_to_delete and table_list:
        original_count = len(table_list)
        updated_table_list = [row for row in table_list if row['Objeto'] not in rows_to_delete]
        session['saved_tables'][active_table_name] = updated_table_list
        session.modified = True
...
def reanalyze():
    ...
    for new_row in new_table_list:
        ...
        if obj_name in old_data_map:
            ...
            for key, new_value in new_row.items():
                if str(new_value) != str(old_row.get(key)):
    ...
    if changed_cell_count > 0:
        ...
        session['last_reanalyze_changes'] = {'summary': summary, 'updated_cells': updated_cells}
    else:
